...
const submitForm = () => {
    if (editingId.value) {
        form.put(route('dashboard.task-priorities.update', editingId.value), {
            preserveScroll: true,
            onSuccess: () => resetForm()
        });
    } else {
        form.post(route('dashboard.task-priorities.store'), {
            preserveScroll: true,
            onSuccess: () => resetForm()
        });
    }
...
const editPriority = (priority) => {
    editingId.value = priority.id;
    form.name = priority.name;
    form.color = priority.color;
    form.level = priority.level;
};
